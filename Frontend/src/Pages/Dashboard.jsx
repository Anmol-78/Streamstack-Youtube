...
function Dashboard() {
    ...
    const handleCheckboxToggle = async(videoId) => {
        try {
            setIsToggling(true);
            const res = await axiosInstance.patch(`/video/toggle/publish/${videoId}`, { videoId });
            console.log(res);
    ...
    const handleUploadVideo = async (videoData, name, videoSize) => {
        try {
            ...
            const res = await axiosInstance.post("/video/publish", videoData);
            if (res.data.statusCode==200) {
                setUploaded(true);
                setUploadingVideo(false);
                setVideoName('');
    ...
    const handleSubmitEditVideo = async (videoData) => {
        try {
            setEditVideo(true);
            const res = await axiosInstance.patch(`/video/v/${editVideoId}`, videoData);
            if (res.data.statusCode==200) {
                setEditVideo(false);
                setEditVideoId('');
                fetchChannelVideos();
        ...
        <>
            <header className="sticky inset-x-0 top-0 z-50 w-full border-b border-white bg-[#121212] px-4">
                ...
                <Link to={"/"}>
                    ...
                    <h3 className="uppercase font-bold text-xl text-white">STREAMSTACK</h3>
                    </div>
                </Link>
                <Search />
